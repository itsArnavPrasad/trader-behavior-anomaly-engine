...
def build_user_profiles():
    ...
    for i in range(N_USERS):
        ...
        # Cycle through anomaly types to guarantee all 10 are represented
...
def empty_base(user_id, ts):
    return {
        ...
        # Timezone gap between home country and login country
        ...
        # Ratio of this trade volume vs user's typical
        "trade_volume_vs_baseline"    : None,
        # Is this a night trade (12am-5am local time)?
        "is_night_trade"              : None,
        # Deposit/withdrawal fields
        "amount"                      : None,
        "method"                      : None,
        # Withdrawal to deposit ratio signal
        "is_immediate_withdrawal"     : None,
        # Session fields
        "session_duration_mins"       : None,
        "page_clicks"                 : None,
        # Clicks per minute — bot detection
        "click_rate_per_min"          : None,
        # KYC fields
        "kyc_change_type"             : None,
        # Account age in days at time of event
...
def events_ip_hopper(user_id, profile, n=8):
    """Rapid IP + country switching — timezone_gap_hours will be high"""
    events = []
...
def events_wash_trader(user_id, profile, n=10):
    """Volume spike 10-20x baseline, single instrument, always profits"""
    events = []
    base_ts = random_timestamp()
    vol = profile["typical_trade_vol"]
    for i in range(n):
...
def events_deposit_withdrawal_cycler(user_id, profile, n=4):
    """Deposit → immediate withdrawal — money laundering pattern"""
    events = []
    base_ts = random_timestamp()
    for i in range(n):
...
def events_bot_trader(user_id, profile, n=8):
    """Inhuman click rate, very short sessions at 2-4 AM"""
    events = []
    base_ts = random_timestamp()
...
def events_structurer(user_id, profile, n=12):
    """Many deposits just under $1000 — classic structuring pattern"""
    events = []
    base_ts = random_timestamp()
    for i in range(n):
...
def events_brute_forcer(user_id, profile, n=6):
    """Escalating failed_attempts then sudden login success"""
    events = []
    base_ts = random_timestamp()
    for i in range(n - 1):
...
def events_dormant_withdrawer(user_id, profile, n=3):
    """Long dormancy then sudden large withdrawal"""
    events = []
...
def events_consistent_winner(user_id, profile, n=10):
    """Always positive PnL, sub-10s trades — latency arbitrage signal"""
    events = []
    base_ts = random_timestamp()
    vol = profile["typical_trade_vol"]
    for i in range(n):
...
def events_device_switcher(user_id, profile, n=8):
    """Different device fingerprint every login"""
    events = []
    base_ts = random_timestamp()
    for i in range(n):
...
def events_kyc_manipulator(user_id, profile, n=4):
    """KYC change immediately followed by large withdrawal"""
    events = []
    base_ts = random_timestamp()
    for i in range(n):
        ts = base_ts + timedelta(hours=i * 2)
        ...
        ts_wit = ts + timedelta(minutes=random.randint(10, 60))
        e_wit  = generate_normal_event(user_id, profile, ts_wit)
        e_wit.update({
            "event_type"  : "withdrawal",
...
def generate_dataset():
    ...
    logger.info(f"Injecting anomaly patterns for {len(anomalous_users)} users...")
    for uid in anomalous_users:
        profile   = profiles[uid]
        generator = ANOMALY_GENERATORS[profile["anomaly_type"]]
        all_events.extend(generator(uid, profile))

    logger.info(f"Generating remaining events to reach {N_EVENTS} total...")
    ...
    logger.info(f"Dataset saved: {out_path}")
    logger.info(f"Total events: {len(df)} | Columns: {len(df.columns)}")
    logger.info(f"Anomalous users: {len(anomalous_users)} / {N_USERS}")
    logger.info(f"Anomalous events: {df['is_anomalous'].sum()} ({df['is_anomalous'].mean()*100:.1f}%)")
